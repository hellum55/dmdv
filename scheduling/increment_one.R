...
if(file.exists("~/dmdv/scheduling/increment_one.rds")){
  # If "increment_one.rds exists then we read it into memory
  increment_one <- readRDS(file = "~/dmdv/scheduling/increment_one.rds")
  # We add one to the R object
  increment_one <- increment_one + 1
  # The R object is saved to the disk
  saveRDS(increment_one, file = "~/dmdv/scheduling/increment_one.rds")  
